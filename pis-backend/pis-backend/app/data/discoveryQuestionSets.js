// Baseline Discovery question sets — B.2 (Template Manager).
//
// IMPORTANT: the `prompt` and `rationale` text below are placeholders so the
// engine has something real to run against. Swap them for whatever the team
// actually agrees on — the counts (8 / 5 / 6) and the suppression wiring are
// the part that matters structurally; the wording isn't final.
//
// `suppressionField` is a dot-path into the Opportunity Context payload
// (Member A's GET /questions/context response) — that's what the suppression
// engine checks to see if we already know the answer.
